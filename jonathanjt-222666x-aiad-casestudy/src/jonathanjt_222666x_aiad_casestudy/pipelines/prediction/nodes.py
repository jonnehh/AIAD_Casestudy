...
def process_miss_data_test(data: pd.DataFrame) -> pd.DataFrame:
...
def feature_engineering_test(data: pd.DataFrame) -> pd.DataFrame:
...
def drop_unnecessary_features_test(data: pd.DataFrame) -> pd.DataFrame:
    # Drop columns that are not needed for modeling
    data.drop(['Name', 'Ticket', 'Cabin', 'PassengerId'], axis=1, inplace=True)
    return data

def predictions(test_data,titanic_data_test):

    with open("data/03_model/current_model.pkl", "rb") as file:
        model = pickle.load(file)
    
    X_test = test_data
   
    predictions = model.predict(X_test)
    prediction_data = titanic_data_test
    prediction_data['Survived_Prediction'] = predictions

    prediction_data.to_csv("data/04_prediction_data/test_data.csv", index=False)
